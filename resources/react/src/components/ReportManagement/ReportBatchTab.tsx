                              ...
                              <div style={{
                                ...
                                <Tooltip title="Xem chi tiết phản hồi">
                                  <Button
                                    ...
                                    onClick={() => {
                                      setSelectedResponseActivity(group.activity || null)
      ...
      <Modal
        title={
          <Space>
            ...
            {selectedResponseActivity && (
              <Text type="secondary" style={{ fontSize: 13 }}>
                - {selectedResponseActivity.title}
              </Text>
            )}
          </Space>
        }
        ...
        onCancel={() => {
          ...
          setSelectedResponseActivity(null)
        }}
        width={900}
        centered
        footer={
      ...
      >
        <div style={{ maxHeight: 500, overflow: 'auto' }}>
          {selectedResponsesForDetail.length > 0 ? (
            selectedResponsesForDetail.map((response, idx) => {
              const orgFiles = response.organization?.id && selectedResponseActivity?.id && detailBatch
                ? getFilesForOrgInActivity(selectedResponseActivity.id, response.organization.id)
                : []
              return (
                <div
                  key={response.id}
                ...
                >
                  ...
                  {orgFiles.length > 0 && (
                    <div>
                      <Text strong style={{ fontSize: 13, color: '#722ed1', display: 'block', marginBottom: 6 }}>
                        <FileOutlined style={{ marginRight: 4 }} />
                        File minh chứng ({orgFiles.length}):
                      </Text>
                      <div style={{
                        ...
                        {orgFiles.map(file => (
                          ...
                          >
                            <Space>
                              {getFileIcon(file.file_type)}
                              <div>
                                <Text style={{ fontSize: 12 }}>{file.title || file.file_name}</Text>
                                <br />
                                <Text type="secondary" style={{ fontSize: 11 }}>
                                  {file.file_name} ({file.file_size_formatted})
                                </Text>
