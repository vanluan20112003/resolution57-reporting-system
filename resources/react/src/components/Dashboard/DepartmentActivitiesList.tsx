...
import { Card, List, Tag, Typography, Space, Input, Select, Button, Badge, Alert, Descriptions, Divider, Segmented, Table } from 'antd'
    ...
    },
    {
      ...
      render: (orgId: string) => (
        <Space>
          ...
          <Text>{mockOrganizations.find(o => o.id === orgId)?.short_name || 'N/A'}</Text>
        </Space>
      ),
    },
    {
  ...
  return (
    <div style={{ padding: '24px' }}>
      <Space direction="vertical" size="large" style={{ width: '100%' }}>
        ...
        {filteredActivities.length > 0 && displayMode === 'table' && (
          <Table
            columns={columns}
        ...
        {filteredActivities.length > 0 && displayMode === 'card' && (
          <List
