...
function OrganizationProfile() {
  ...
  // Responsive avatar size: 220px on mobile (xs), 100px on desktop
  // screens.xs is true when viewport < 576px, undefined initially
  const isMobile = screens.xs && !screens.sm
  const avatarSize = isMobile ? 220 : 100
  ...
  const [membersRoleFilter, setMembersRoleFilter] = useState<
    string | undefined
  >()
  ...
  const handleUpdateMemberRole = async (
    ...
    try {
      ...
      message.success(
        `Đã ${newRole === "STAFF" ? "thăng cấp" : "hạ cấp"} thành viên thành công`
      )
      fetchMembers()
    } catch (error: any) {
  ...
  const canEdit =
    user && ["STAFF", "MANAGER", "OPERATOR", "ADMIN"].includes(user.role || "")

  // Can current user change member roles? (MANAGER, OPERATOR, ADMIN)
  const canChangeRoles =
    user && ["MANAGER", "OPERATOR", "ADMIN"].includes(user.role || "")
      ...
      ? [
          {
            ...
            render: (_: any, record: OrganizationMember) => {
              ...
              const canRemoveThis =
                record.role !== "MANAGER" || user?.role !== "MANAGER"
                  ...
                  {canChangeThisRole &&
                    (record.role === "GUEST" ? (
                      <Popconfirm
                        ...
                        onConfirm={() =>
                          handleUpdateMemberRole(record.id, "STAFF")
                        }
                        okText="Thăng cấp"
                        cancelText="Hủy">
                      ...
                      <Popconfirm
                        ...
                        onConfirm={() =>
                          handleUpdateMemberRole(record.id, "GUEST")
                        }
                        okText="Hạ cấp"
                        cancelText="Hủy"
  ...
  const renderActivitiesTab = () => (
    <Card>
      <div style={{ textAlign: "center", padding: 60 }}>
        ...
        <Text type="secondary">
          Chức năng này đang được phát triển. Vui lòng truy cập menu "Quản lý
          hoạt động" để xem danh sách hoạt động.
        </Text>
      </div>
    </Card>
  )
        ...
        <div className="org-header-section">
          ...
          <div className="org-header-info">
            ...
            <Space wrap>
              ...
              <Tag
                color={
                  organization.status === "active" ? "success" : "default"
                }>
